...
public class EmailService {
    ...
    public int notifyMatchingDonors(EmergencyEmailRequest request) {
        try {
    ...
    private void sendEmergencyEmail(String donorEmail, EmergencyEmailRequest request) {
        try {
    ...
    private String createEmailContent(EmergencyEmailRequest request) {
    ...
    public void sendAppointmentStatusEmail(NotificationDTO notificationDTO) {
        try {
            MimeMessage message = mailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(message, true);
    ...
    private String createShortMessage(NotificationDTO notification) {
        return String.format("Appointment %s - Please check your email for details.",
                notification.getType() == Notification.NotificationType.APPOINTMENT_CONFIRMED ? "confirmed" : "cancelled");
    }

    private String createEmailContent(NotificationDTO notification) {
        String statusColor = notification.getType() == Notification.NotificationType.APPOINTMENT_CONFIRMED
                ? "#28a745" : "#dc3545";
